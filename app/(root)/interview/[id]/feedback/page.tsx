    ...
    <section className="section-feedback">
      <div className="flex flex-col items-center gap-3 text-center">
        ...
        <p className="text-xl text-light-400">
          <span className="capitalize text-primary-200 font-semibold">{interview.role}</span> Interview Performance Analysis
        </p>
      </div>
      ...
      <div className="flex flex-col gap-6">
        <h2 className="text-3xl font-bold tracking-tight">Performance Breakdown</h2>
        <div className="grid gap-4">
          {feedback?.categoryScores?.map((category, index) => (
            <div key={index} className="p-6 rounded-xl bg-gradient-to-br from-dark-200/80 to-dark-300/80 border border-dark-400/30 hover:border-primary-200/30 transition-all duration-300">
              ...
              <p className="text-light-200 leading-relaxed">{category.comment}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="flex flex-col gap-4 p-6 rounded-2xl bg-gradient-to-br from-success-100/5 to-transparent border border-success-100/20">
        <h3 className="text-2xl font-bold tracking-tight text-success-100">✓ Strengths</h3>
        <ul className="space-y-2">
          {feedback?.strengths?.map((strength, index) => (
            <li key={index} className="text-light-200 leading-relaxed">{strength}</li>
          ))}
        </ul>
      </div>
      ...
      <div className="flex flex-col gap-4 p-6 rounded-2xl bg-gradient-to-br from-accent-100/5 to-transparent border border-accent-100/20">
        <h3 className="text-2xl font-bold tracking-tight text-accent-100">→ Areas for Improvement</h3>
        <ul className="space-y-2">
          {feedback?.areasForImprovement?.map((area, index) => (
            <li key={index} className="text-light-200 leading-relaxed">{area}</li>
          ))}
        </ul>
      </div>
